...
1. 调试模块是textarea数据包是字符串格式，换行是`\n`，而在repeater里是http包格式，所以在repeater、proxy模块中会有`\r\n`，在获取字符串的时候可以进行`\r\n`的替换即可
...
## 0x16 使用插件在`Proxy`模块无法进行解密？复杂明文、密文或者请求包、响应包密文不同情况下，怎么自动解密？

这是预期行为，插件不会在Proxy模块进行自动解密，这会损坏应用系统的加密、解密逻辑，导致在一些情景下，比如点击APP的按钮或者触发一个登录页面的时候，`Proxy`的模块如果自动解密了，后续的报文无法进行了，所以本插件预期逻辑不会在`Proxy`下解密；

非预期情况下，可以通过配置关键字达到这个效果，用接口，全部内容都抛入接口解密，用try和except包裹，符合密文特征的走try逻辑，不符合的直接except返回原文，另外域名设置为明文关键字，然后选中明文关键字应用于请求头，这样就可以达到自动解密的效果了

<img width="780" alt="image" src="https://github.com/user-attachments/assets/7a949d4b-5c53-4a98-b014-2e090d48a6ed" />
