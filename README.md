...
## 2026.4.14  update 0.60
1. 新增`autodecoder-E`扩展选项卡，只读权限，查看密文，不需要去logger模块看加密【`autodecoder`扩展选项卡默认为解密，不再判断明文关键字或者密文关键字】
2. 新增菜单选项`数据包关键字判断`，自动判断关键字是否满足需求
3. 新版burp支持`autodecoder`扩展选项卡修改明文后自动加密
4. 自带加解密增加`Recipe模块`，类`cyberchef`，也可以满足日常测试加解密参数等
5. 自带加解密增加调试模块，方便对自带加解密进行详细调试
6. `Proxy`模块支持修改明文后，插件自动加密，方便手动修改`Proxy`模块进行测试

## 2025.12.14  update 0.57
1. 修改请求头关键字增加适配自带加解密
2. 优化扩展选项卡适配自带加解密的请求头方式解密
3. 增加保存配置时的加解密方法选择确认
4. 修复未知版本无法显示保存配置界面问题
5. 优化插件按钮排列，防止混用

## 2025.11.19  更新0.56
1. 更新English版本，非常感谢@secfb  #48 
2. 修复sm2解密问题 #47 
3. 增加请求体base64自动解码，适配加密内容为二进制的数据，感谢微信群师傅 @满溢
